...
* **AI 활용 상세 문서:** [`AI_USAGE.md`](./AI_USAGE.md)
...
* 일상에서 바로 수행할 수 있는 일상형 미션 제공
* 사용자 주변 장소를 활용하는 장소형 미션 제공
...
Begin Again은 AI를 **서비스 핵심 기능**과 **개발 과정** 모두에 활용했습니다.
...
서비스 기능 구현뿐 아니라 기획, 설계, 개발, 디버깅, 문서화 과정에서도 코딩 AI를 활용했습니다.

#### 주요 활용 방식

* 서비스 아이디어와 사용자 여정 구체화
* 데이터베이스 및 백엔드 구조 설계 검토
* Expo, React Native, TypeScript 코드 작성 및 리팩터링
* Supabase Edge Functions와 RPC 구현 보조
* Upstage Solar API 프롬프트 설계
* TypeScript 및 ESLint 오류 원인 분석
* Git 브랜치, 충돌 및 배포 오류 해결
* 통합 테스트와 AI 응답 검증 로직 작성
* README, AI 활용 문서 및 제출 자료 정리

#### AI 작업 규칙

AI가 프로젝트 구조를 임의로 변경하거나 민감정보를 커밋하지 않도록 `AGENTS.md`에 작업 규칙을 작성해 활용했습니다.

* 원본 저장소가 아닌 팀 포크 저장소에서 작업
* 기존 설정과 팀원의 변경 사항 보존
* API Key 및 서버 비밀값 커밋 금지
* 수정 후 TypeScript 검사와 ESLint 실행
* 사용자가 기록하지 않은 사건이나 감정을 AI 결과에 추가하지 않도록 제한

#### 개발 결과 검증

AI가 생성한 코드는 그대로 적용하지 않고 다음 과정을 거쳐 검증했습니다.

1. 변경된 코드와 기존 프로젝트 구조 비교
2. `npx tsc --noEmit`을 통한 타입 검사
3. `npm run lint`를 통한 정적 분석
4. 웹 및 Expo 환경에서 주요 기능 직접 실행
5. AI 응답의 필드, 배열 개수 및 `recordIndex` 검증
6. 사용자가 입력하지 않은 정보가 결과에 포함되지 않았는지 확인

AI는 개발 속도를 높이는 보조 도구로 활용했으며, 최종 적용 여부는 코드 검사와 실제 실행 결과를 바탕으로 판단했습니다.

### AI 관련 파일 및 구현 위치

| 경로                             | 설명                               |
| ------------------------------ | -------------------------------- |
| [`AGENTS.md`](./AGENTS.md)     | 코딩 에이전트 작업 및 보안 규칙               |
| [`AI_USAGE.md`](./AI_USAGE.md) | AI 활용 과정과 결과 검증 상세 기록            |
| `supabase/functions/`          | AI 미션 추천 및 에세이 생성 Edge Functions |
| `services/`                    | 클라이언트와 서버 함수 사이의 요청·응답 처리        |
| `backend/`                     | AI 응답 검증 및 통합 테스트                |

API Key, 비밀번호 등 민감정보가 포함된 파일은 저장소에 커밋하지 않았습니다.
